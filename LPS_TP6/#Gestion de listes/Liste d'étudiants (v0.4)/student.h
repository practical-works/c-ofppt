...
void menu_program(int *choix) {
	printf("\n");
	printf("\t\t*============================================*\n");
	printf("\t\t| 1. NOUVELLE LISTE    | 8. Tri par Alphabet |\n");
	printf("\t\t|----------------------| 9. Tri par Moyenne  |\n");
	printf("\t\t| 2. Meilleure Moyenne |---------------------|\n");
	printf("\t\t| 3. Mention           |                     |\n");
	printf("\t\t|----------------------| 10. SAUVER LISTE    |\n");
	printf("\t\t| 4. Rechercher        | 11. CHARGER LISTE   |\n");
	printf("\t\t| 5. Ajouter           |                     |\n");
	printf("\t\t| 6. Modifier          |---------------------|\n");
	printf("\t\t| 7. Supprimer         | 12. QUITTER         |\n");
	printf("\t\t*============================================*\n");
	printf("\n * Choix: ");
	scanf("%d", &(*choix));
}
...
void modifier(struct etudiant list[], int dim) {
	int i, code, code_id, code_trouve = 0, modif;
	...
	if (code_trouve == 0) {
		...
		menu:
		system("cls"); titre_program();
		afficher_etudiant(list,code_id);
		printf("\n");
		printf("\t\t\t*------------------------*\n");
		printf("\t\t\t| 0. Modifier Nom/Prenom |\n");
		printf("\t\t\t|------------------------|\n");
		printf("\t\t\t| 1. Modifier Note 1     |\n");
		printf("\t\t\t| 2. Modifier Note 2     |\n");
		printf("\t\t\t| 3. Modifier Note 3     |\n");
		printf("\t\t\t|------------------------|\n");
		printf("\t\t\t| 4. RETOUR              |\n");
		printf("\t\t\t*------------------------*\n");
		printf("\n * Choix: ");
		scanf("%d", &modif);
		switch (modif) {
			case 0:	printf("\n\t°°° Nom/Prenom: "); fflush(stdin); gets(list[code_id].nom); break;
			case 1:	printf("\n\t°°° Note 1: "); scanf("%f", &list[code_id].note1); 			break;
			case 2:	printf("\n\t°°° Note 2: "); scanf("%f", &list[code_id].note2); 			break;
			case 3:	printf("\n\t°°° Note 3: "); scanf("%f", &list[code_id].note3); 			break;
			case 4: return;
			default: choix_invalide(); system("pause>nul");
		} 
		list[code_id].moyenne = (list[code_id].note1 + list[code_id].note2 + list[code_id].note3) / 3;
		fflush(stdin); goto menu;
	}
}
